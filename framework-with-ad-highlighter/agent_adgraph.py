...
import subprocess
import sys
from autofr.common.adgraph_version import DEFAULT_DOCKER_NAME_ADGRAPH_NEW, DEFAULT_DOCKER_NAME_ADGRAPH_OLD, set_adgraph_version
...
from autofr.rl.browser_env.browser_adgraph_env import AdgraphBrowserWithAdHighlighter
from autofr.rl.browser_env.browser_env import GOOGLE_TMP_URL
...
if __name__ == "__main__":
    ...
    parser.add_argument('--adblock_ext_path', required=False,
                        help='Path to adblock plus extension')
    parser.add_argument('--adblock_proxy_path', required=False,
    ...
    parser.add_argument('--disable_isolation',
                        default="True",
                        help='Disable site isolation for browser')
    parser.add_argument('--adgraph_version',
                        type=str,
                        choices=[DEFAULT_DOCKER_NAME_ADGRAPH_OLD, DEFAULT_DOCKER_NAME_ADGRAPH_NEW],
                        default=DEFAULT_DOCKER_NAME_ADGRAPH_OLD,
                        required=False,
                        help='Version of AdGraph to use for site snapshots')
    ...
    disable_isolation = str(args.disable_isolation).lower() == "true"

    set_adgraph_version(args.adgraph_version)
    ...
    with AdgraphBrowserWithAdHighlighter(args.url,
        ...
        logging.basicConfig(handlers=[logging.FileHandler(agent.output_path + os.sep + "log.log", mode="w"),
                                      logging.StreamHandler()],
                            format=format_str, level=args.log_level.upper())
        #logger.debug(f"AdgraphBrowserWithAdHighlighter {args.chrome_driver_path}, {args.browser_path}")
