...
const { truncatedNormal } = require("@tensorflow/tfjs-core");
...
if (document instanceof HTMLDocument)
{
  ...
  //.log("Attaching get_abp_hitrecords");
  document.addEventListener("setHitRecordinDOM", event => {
    //console.log(event);
    //console.log("asking for getHitRecords");
    browser.runtime.sendMessage({
      type: "hitLogger.getHitRecords"
    }).then(response =>
    {
      console.log("Got the hit records: ");
      console.log(response);
      let el_id = "abp-hitrecords";
      ...
      el.textContent = JSON.stringify(response);

      return true;
    });
  }, true);

  document.addEventListener("resetHitRecords", event => {
    //console.log(event);
    //console.log("asking for resetHitRecords");
    browser.runtime.sendMessage({
      type: "hitLogger.resetHitRecords"
    });
  }, true);

  /* Important, this must be here */
  document.addEventListener("update_rl_filter_list", event => {
    //console.log(event);
    //console.log("asking for resetHitRecords");
    browser.runtime.sendMessage({
